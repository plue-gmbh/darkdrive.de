...
function shareSave(files){var tok=Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8);return caches.open(SHARED).then(function(c){var saved=0;function step(i){if(i>=files.length)return Promise.resolve();var f=files[i];return c.put('/__shared/'+tok+'-'+i+'/'+encodeURIComponent(f.name),new Response(f,{headers:{'Content-Type':f.type||'application/octet-stream'}})).then(function(){saved++;return step(i+1)})}return step(0).then(function(){return{saved:saved,total:files.length,err:''}},function(err){return{saved:saved,total:files.length,err:(err&&err.name)||'CacheError'}})})}
function shareResult(r){if(!r.err)return Response.redirect('/?shared='+r.saved,303);return Response.redirect('/?share_failed='+encodeURIComponent(r.err)+'&saved='+r.saved+'&of='+r.total,303)}
self.addEventListener('fetch',function(e){var req=e.request;if(req.method==='POST'&&new URL(req.url).pathname==='/share'){e.respondWith(req.formData().then(function(fd){return shareSave(fd.getAll('upload').filter(function(f){return f&&f.size}))}).then(shareResult).catch(function(err){return Response.redirect('/?share_failed='+encodeURIComponent((err&&err.name)||'ShareError'),303)}));return}if(req.method!=='GET')return;if(req.mode==='navigate'){e.respondWith(fetch(req).catch(function(){return caches.match('/offline')}));return}var path=new URL(req.url).pathname;var isShell=SHELL.some(function(s){return path===s||path.startsWith(s+'?')});if(isShell){e.respondWith(fetch(req).then(function(resp){var clone=resp.clone();caches.open(CACHE).then(function(c){c.put(req,clone)});return resp}).catch(function(){return caches.match(req,{ignoreSearch:true})}))}});
<?php
    exit;
  }
